...
fn test_index_sorting_complexity() {
    ...
    assert!(index[0] <= index[1], "Hashes must sort sequentially for O(log N) lookup binary-search compliance");
}
...
fn test_256_byte_padding_math() {
    ...
    assert_eq!(padding_needed, 155, "Padding math calculation anomaly detected");
    assert_eq!((current_cursor + padding_needed) % ALIGNMENT_BOUNDARY, 0, "SIMD Alignment calculation failed!");
}
